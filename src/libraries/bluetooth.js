...
class XsensDot {
  ...
  readMessageAck(dataViewObject) {
    return this.getCharacteristicData(
      ...
      if (this.verbose) {
        ...
        let str = `${cmd} ack: ${getKeyByValue(
          msgAckEnum,
          value.getUint8(3, true)
        )}`;
        console.log(str);
      }
      return value;
  ...
  writeDeviceName(name) {
    return this.getCharacteristicData(
      ...
      .then((value) => {
        ...
        this.verbose = false;
        return this.writeCharacteristicData(
          serviceEnum.configuration_service,
          serviceEnum.device_control,
          value
        ).then(() => {
          this.verbose = true;
          return this.readDeviceName().then((res) => {
    ...
    return this.getCharacteristicData(
      ...
      .then((value) => {
        ...
        for (
          ...
          index < value.getUint8(7, true) + startOffset;
          index++
        ) {
  ...
  downloadDataToCSV() {
    ...
    let downloadArray = [["Qx", "Qy", "Qz", "Qw", "E", "N", "U", "T"]].concat(
      this.data
    );
    ...
    downloadArray.forEach(function (rowArray) {
      ...
      for (let i = 0; i < rowArray.length; i++) {
        if (rowArray[i].constructor.name === "Quaternion") {
          tmpArr.push(
            rowArray[i].x.toString(),
            rowArray[i].y.toString(),
            rowArray[i].z.toString(),
            rowArray[i].w.toString()
          );
    ...
    console.log(
      `Amount of data points: ${downloadArray.length}, amount of columns for each data point: ${downloadArray[0].length}`
    );
