...
class XsensDot {
  ...
  getBatteryLevel() {
    return this.getCharacteristicData(
      ...
      .then((value) => {
        console.log(`Battery percentage: ${value.getUint8(0, true)}`);
...
function findBluetoothDevices() {
  XsensDotSensor.request()
    ...
    .then(() => {
      XsensDotSensor.readDeviceName();
    })
    .then(() => {
      XsensDotSensor.getBatteryLevel();
    })
    .then(() => {
  ...
  XsensDotSensor.subscribeToCharacteristicChangedNotifications(
    ...
    .then(() => {
      ...
      let dataViewObject = XsensDotSensor.createMessageObject(
        recMsgTypeEnum.recording_message,
        0,
        recMsgEnum.requestFlashInfo,
        []
      );
  ...
  XsensDotSensor.subscribeToCharacteristicChangedNotifications(
    ...
    .then(() => {
      ...
      let dataViewObject = XsensDotSensor.createMessageObject(
        recMsgTypeEnum.recording_message,
        0,
        recMsgEnum.requestRecordingTime,
        []
      );
